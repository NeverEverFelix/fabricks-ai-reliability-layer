...
describe("runIntent – fallback behavior", () => {
    it("uses the fallback step when the primary step fails", async () => {
        ...
        const primaryFinished = telemetryEvents.find((e) => e.type === "step_finished" && e.stepId === "primary");
        expect(primaryFinished).toBeDefined();
        expect(primaryFinished.success).toBe(false);
        const fallbackStarted = telemetryEvents.find((e) => e.type === "step_started" && e.stepId === "fallback");
        expect(fallbackStarted).toBeDefined();
        const fallbackFinished = telemetryEvents.find((e) => e.type === "step_finished" && e.stepId === "fallback");
        expect(fallbackFinished).toBeDefined();
        expect(fallbackFinished.success).toBe(true);
        const intentFinished = telemetryEvents.find((e) => e.type === "intent_finished");
...
describe("runIntent – retry behavior", () => {
    it("retries a failing step and succeeds on a later attempt", async () => {
        ...
        // 2) Telemetry expectations
        const retryStarted = telemetryEvents.filter((e) => e.type === "retry_attempt_started");
        const retryFailed = telemetryEvents.filter((e) => e.type === "retry_attempt_failed");
        // Attempt 1 + Attempt 2
...
describe("runIntent – timeout behavior", () => {
    it("fails when a step exceeds timeoutMs and surfaces the timeout as a failed step + intent", async () => {
        ...
        const slowStepFinished = telemetryEvents.find((e) => e.type === "step_finished" && e.stepId === "slow-step");
...
describe("runIntent – integration with providers", () => {
    it("can call a provider from a step and return its result", async () => {
        ...
        const intent = defineIntent({
            ...
            steps: [
                ...
                },
                {
                    id: "call-llm",
                    async run(ctx) {
                        const prompt = ctx.metadata?.prompt;
        ...
        // Engine result
        expect(result.success).toBe(true);
        expect(result.output).toBe('LLM: ANSWER THIS QUESTION CLEARLY: WHAT IS RELIABILITY?');
