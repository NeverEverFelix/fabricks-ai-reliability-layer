...
export async function runIntent(intent, ctx) {
    ...
    const entryStepId = intent.entryStepId;
    let stepToRun;
    if (entryStepId) {
    ...
    try {
        const output = await runWithRetry(() => runWithTimeout(() => stepToRun.run(ctx), stepToRun.timeoutMs, ctx.telemetry, name, stepToRun.id), ctx.telemetry, name, stepToRun.id, stepToRun.retry);
        emit({
            type: "step_finished",
            intentName: name,
            stepId: stepToRun.id,
        ...
        try {
            const fallbackOutput = await runWithRetry(() => runWithTimeout(() => fallbackStep.run(ctx), fallbackStep.timeoutMs), ctx.telemetry, name, fallbackStep.id, fallbackStep.retry);
            emit({
                type: "step_finished",
                intentName: name,
                stepId: fallbackStep.id,
